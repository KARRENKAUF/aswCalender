...
func highlightBoxHTML() string {
	...
	if err != nil {
		loc = time.UTC // Fallback falls Zeitzone nicht geladen werden kann
	}
	// format current time
	lastGeneratedTime := time.Now().In(loc).Format("02/01/2006 15:04") // dd/mm/yyyy HH:MM
...
func renderPage(path, title, subtitle string, blocks fileGroup, blockOrder []string, showToolbar bool, navToAll bool, navToIndex bool) error {
	...
	b.WriteString("<a class='navlink secondary' href='" + html.EscapeString(sourcePage) + "'>Source page (ASW)</a>")
	b.WriteString("</div>")
...
func renderGoogleHelpPage(path string) error {
	...
	b.WriteString("<ul>")
	b.WriteString("<li><div class='row'><div class='row-left'>")
	b.WriteString("<div class='file'>Tap “Download file”</div>")
